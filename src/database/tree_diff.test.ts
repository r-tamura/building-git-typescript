...
const commit = (treeOid: string) => new Commit([], treeOid, {} as any, {} as any, "");
...
describe("TreeDiff#compareOids", () => {
  ...
  describe("a/bが異なるOIDのとき、変更点がある", () => {
    it.each([
      [
        ...
        [new Entry("3a3c4ecxxx", 0o0100644), new Entry("3a3c4ecxxx", 0o0100755)],
      ],
      [
        "contentsが異なる",
        "3a3c4ecaaa",
        ...
        [new Entry("3a3c4ecxxx", 0o0100644), new Entry("3a3c4ecyyy", 0o0100644)],
      ],
    ])("%s", async (_title, a_oid, a_tree, b_oid, b_tree, expected) => {
      // Arrange
      const mockedLoad = jest.fn().mockResolvedValueOnce(a_tree).mockResolvedValueOnce(b_tree);
      const db = {
        load: mockedLoad,
      } as any;
  ...
  it("aのOIDが存在し、bがnullのとき、変更点がある", async () => {
    ...
    // Assert
    assert.deepEqual(diff.changes.get("hello.txt"), [new Entry("3a3c4ecaaa", 33188), null]);
  });
  ...
  it("aがnull、bのOIDが存在するとき、変更点がある", async () => {
    ...
    // Assert
    assert.deepEqual(diff.changes.get("hello.txt"), [null, new Entry("3a3c4ecaaa", 33188)]);
  });

  describe("フィルター", () => {
    const three_a = new Entry("17eb192f8d22d50442f606b1a3ddeaec3ee8e08b", 0o100644);
    const two_a = new Entry("b17eb192f8d22d50442f606b1a3ddeaec3ee8e08", 0o100644);
    const two_b = new Entry("4b894fc2fc977ccc2ae8195d1ba60213b1111e48", 0o100644);
    const one_a = new Entry("5436437fa01a7d3e41d46741da54b451446774ca", 0o100644);
    const one_b = new Entry("0a0c575ef52ed277c7d5fea890c0ca5a18d63ed5", 0o100644);
    const objects = {
